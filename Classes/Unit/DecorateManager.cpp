...
MakeEatLayer* DecorateManager::getEatableLayer()
{
    ...
    MyRenderTexture* rt = MyRenderTexture::create(visibleSize.width, visibleSize.height);
    rt->setPosition(visibleSize*0.5);
    rt->beginWithClear(0, 0, 0, 0);
    if (m_pFoodLayer) {
        m_pFoodLayer->visit();
...
void DecorateManager::_updateFinishedPercent()
{
    ...
    if ((curPercent*100/percent>=98 || percent==0) && (curPercent1*100/percent1>=98 || percent1==0)&& (curPercent2>90 || curPercent2 ==0)) {
        m_bFinishedEat = true;
    }
}
