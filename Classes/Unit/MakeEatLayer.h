...
public:
    ...
    void setRenderTexture(RenderTexture* rt){
        ...
        m_pRenderTexture->setPosition(Vec2(m_pRenderTexture->getSprite()->getContentSize().width*0.5, m_pRenderTexture->getSprite()->getContentSize().height*0.5)); addChild(m_pRenderTexture);
...
protected:
    MakeEatLayer():m_pRenderTexture(NULL),m_pShapeSprite(NULL),m_pDelegate(NULL),m_pFunc(NULL){
