...
std::string FileUtility::getFileDirectory(std::string fileName)
{
...
bool FileUtility::isFileExist(std::string filePath)
{
...
bool FileUtility::createFile(std::string filePath)
{
	if(filePath.size() < 1) return false;
...
bool FileUtility::removeDir( string path )
{
	#if (CC_TARGET_PLATFORM != CC_PLATFORM_WIN32)
	DIR *dir;
	dirent *dir_info;
