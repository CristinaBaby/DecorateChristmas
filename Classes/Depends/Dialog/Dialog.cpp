...
bool Dialog::init()
{
	if (LayerColor::init())
    {
		...
		contentLayer=LayerColor::create(Color4B(255,255,255,160));
        ...
        if (m_bIsCocos) {
            if(dialogType==DIALOG_TYPE_SINGLE){
                ...
                this->addChild(m_pUL);
                Button* pOKButton = dynamic_cast<Button*>(cocos2d::ui::Helper::seekWidgetByName(m_pUL, "popup_btn_ok"));
                pOKButton->addTouchEventListener(CC_CALLBACK_2(Dialog::_onUIButtonCallback, this));
                pOKButton->setVisible(true);
                
                Button* pYESButton = dynamic_cast<Button*>(cocos2d::ui::Helper::seekWidgetByName(m_pUL, "popup_btn_yes"));
                ...
                Button* pNOButton = dynamic_cast<Button*>(cocos2d::ui::Helper::seekWidgetByName(m_pUL, "popup_btn_no"));
...
//            bg->setAnchorPoint(Vec2(0.5,0.5));
            ...
            title_label = Label::createWithTTF("",font_felt.c_str(), fontsize(40),Size(bg->getContentSize().width*0.9, bg->getContentSize().height*0.4),TextHAlignment::CENTER, TextVAlignment::CENTER);
            title_label->setPosition(Vec2(contentLayer->getContentSize().width/2,bg->getContentSize().height*0.75));
            bg->addChild(title_label);
            
            content_label = Label::createWithTTF("",font_felt.c_str(), fontsize(30));
            content_label->setDimensions(dialogSize.width*0.8,dialogSize.height*0.8);
            
            
            content_label->setAnchorPoint(Vec2(0.5,1));
            content_label->setColor(Color3B(255,255,218));
            ...
            if(dialogType==DIALOG_TYPE_SINGLE)
            {
                positive= MenuItemSprite::create(Sprite::create("dialog/popup_btn_yes.png"), Sprite::create("dialog/popup_btn_yes.png"),
            ...
            positive_label->setColor(Color3B(255,255,218));
            ...
            negative_label->setColor(Color3B(255,255,218));
...
void Dialog::onClick(Ref* obj)
{
    ...
    if(type==MENU_TYPE_1)
    {
        ...
        FiniteTimeAction* action = Sequence::create(ScaleTo::create(0.3,0.01),CallFunc::create(CC_CALLBACK_0(Dialog::dismiss,this)) , NULL);
        
        bg->runAction(action);
        
    }else
    {
        FiniteTimeAction* action = Sequence::create(ScaleTo::create(0.3,0.01),CallFunc::create(CC_CALLBACK_0(Dialog::onBack,this)) , NULL);
...
void Dialog::setContentText(const char* text)
{
    if (!m_bIsCocos) {
        ...
        Text* pText = dynamic_cast<Text*>(cocos2d::ui::Helper::seekWidgetByName(m_pUL, "text"));
        Text *pShadow = dynamic_cast<Text*>(cocos2d::ui::Helper::seekWidgetByName(m_pUL, "text_shadow"));
...
{
	content_label->setPosition(Vec2(dialogSize.width/2,dialogSize.height*0.8-title_label->getContentSize().height));
}
